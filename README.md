...
3. Within about 18 seconds the signal decodes and a tap card appears with the URL. A live debug log under the mic level shows what the decoder is doing in real time.

You can also play the encoded WAV through a real FM radio, a podcast, or a TV ad to test the broadcast path.

## How it works

- **Modulation:** 8-FSK in the 1200 to 1900 Hz audible band. Each 50 ms symbol carries 3 bits. A 500 ms chirp sweep (500 to 2500 Hz) marks the start of each frame, and the frame is sent twice with its own chirp each time so a listener who starts halfway through still gets a clean catch.
...
- Reed Solomon forward error correction (CRC-16 only)
